...
var fromPath = module.exports.path = function(imagePath,options,callback){
  ...
  var caption = options.caption 
    ...
    , argumetz = options;
  //console.log(caption);
  //console.log(argumetz+"<---initial argumetz")
  generate(argumetz,imagePath, caption, {top : !!bottomCaption}, function(err,captioned,original){
    if (err) callback(err)
    debug('generated image (%s), cleaning up.', captioned)
    cleanup(outputFile,err,captioned,original,function(err,image){
      if(bottomCaption){
        generate(argumetz,image,bottomCaption,{},function(err,captioned,original){
